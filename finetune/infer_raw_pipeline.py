...
def get_rotate_crop_image(img, points):
    """PaddleOCR tools/infer/utility.py에서 그대로 가져옴 — 회전/기운 quad를
    perspective warp으로 똑바로 편 crop 만듦. 세로로 길면(문자가 세로쓰기) 90도 돌림."""
...
def resolve_orientation(rec_model, rec_post, rec_ops, crops, flip_vote_thresh=0.5):
    ...
    # 2단계: crop별 0/90/270 확인 (0도는 1단계에서 이미 계산한 값 재사용해서 낭비 안 함)

...
사용법:
  python pipeline/eval_paddle.py                  # data/synth 전체 (repo 루트에서 실행)
  python pipeline/eval_paddle.py data/synth/images/drawing_0003.png
"""
...
def build_ocr():
    ...
    # rec: v5_mobile -> v6_small 교체. v5 사전은 원문자 ⑮/⑳ 등 일부 특수기호
    # 누락, v6_tiny는 GD&T 핵심기호(↧/⏤/⏥/⌭)째 빠짐 -> v6_small이 사전
    # 전체(①~⑳ 포함) 지원하면서 medium보다 가벼운 티어라 선택.
    # det은 이번 파인튜닝 대상 아니라 기존 v5_mobile_det 유지.
    return PaddleOCR(
